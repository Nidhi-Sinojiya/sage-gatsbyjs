                    ...
                    <SwiperSlide key={i}>
                      ...
                      <div className="testimonialbtm relative mt-50 inline-block">
                        <div className="relative flex flex-wrap gap-x-[30px] justify-center">
                          {item?.title && (
                            <span className="italic text-green-100 text-14 -tracking-[0.24px] font-400 font-primary">
                              {parse(item.title)}
                            </span>
                          )}
                          {item?.testimonialCategory.nodes.length > 0 &&
                            item.testimonialCategory.nodes.map((itemCat, i) => {
                              return (
                                <span
                                  className="text-green-100 text-12 tracking-[0.2px] font-420 uppercase"
                                  key={i}
                                >
                                  {itemCat.name}
                                </span>
...
export const PageTestimonialListingFragment = graphql`
  fragment PageTestimonialListingFragment on WpPage_Pagecontent_PageContent_TestimonialListing {
    extraId
    extraClass
    hideSection
    heading
